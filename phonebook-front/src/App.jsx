...
const App = () => {
  ...
  useEffect(() => {
    ...
    PersonService.getAll()
      .then((alustaPersons) => {
        console.log("RAW DATA:", alustaPersons);
        console.log("TYPE:", typeof alustaPersons);
        console.log("IS ARRAY:", Array.isArray(alustaPersons));
  ...
  const addPerson = (event) => {
    ...
    if (personExists(personObject.name)) {
      ...
      if (
        window.confirm(
          `${foundPerson.name} is already added to the phonebook, would you like to update their phone number?`,
        )
      )
        // this is their number ${foundPerson.name}
        // and this is their id ${foundPerson.id}`))
        PersonService.update(foundPerson.id, personObject).then((vastaus) => {
          //console.log(vastaus, 'vastauksen testaus', vastaus.id, '=tämä on vastauksen id')
          setPersons(
            persons.map((person) =>
              person.id !== vastaus.id ? person : vastaus,
            ),
          );
          setNotification({
            message: `${foundPerson.name} Number updated successfully!`,
            type: "success",
          });
    ...
    const newId =
      persons.length > 0
        ? (Math.max(...persons.map((p) => parseInt(p.id))) + 1).toString()
        : "1";
    if (!personExists(personObject.name)) {
      PersonService.create({ ...personObject, id: newId })
        .then((person) => {
